...
public class BTreeFile extends IndexFile implements GlobalConst {
...

	{
		...
		if (headerPage.get_maxKeySize() < BT.getKeyLength(key)) {
			throw new KeyTooLongException(null, "");
		}

		// @exception KeyNotMatchException
		if (!(key instanceof StringKey) && !(key instanceof IntegerKey)) {
			throw new KeyNotMatchException(null, "");
		}

		if (key instanceof StringKey && headerPage.get_keyType() != AttrType.attrString) {
			throw new KeyNotMatchException(null, "");
		}

		if (key instanceof IntegerKey && headerPage.get_keyType() != AttrType.attrInteger) {
			throw new KeyNotMatchException(null, "");
		}

		// The tree is empty and we need to create a new first page which will be a leaf
		// page
		...
		// The tree is not empty and we call _insert method to insert key and rid pair.
		else {
			// Catching the return value from _insert method
			// It contains information about the key and PageId pair
			KeyDataEntry newRootEntry = _insert(key, rid, headerPage.get_rootId());

			// newRootEntry != null means that a leaf split occurred that reached to
			// the header page and the header page got split too.
			if (newRootEntry != null) {
				...
				// The old root page is the left child of the newly created root
	...
	private KeyDataEntry _insert(KeyClass key, RID rid, PageId currentPageId)
		...
		if (currentPage.getType() == NodeType.INDEX) {
			...
			// A split occurred and upEntry has the data about it
			if (upEntry != null) {
				page = pinPage(currentPageId);
				currentIndexPage = new BTIndexPage(page, headerPage.get_keyType());

				// Current index page can has space available to hold this entry
				...
				// NO space available on the current index page. Create a new index page
				// and distribute the entries
				{
					BTIndexPage newIndexPage = new BTIndexPage(headerPage.get_keyType());
					...
					// Distributing all the records from currentIndexPage to newIndexPage
					for (tmpkeyDataEntry = currentIndexPage.getFirst(delRID); newIndexPage
							.available_space() > currentIndexPage
									.available_space(); tmpkeyDataEntry = currentIndexPage.getFirst(delRID)) {
					...
					// Checking whether the upEntry data pair goes to the index or the current page
					// Goes to the newIndexPage
					if (BT.keyCompare(upEntry.key, (newIndexPage.getFirst(new RID())).key) >= 0) {
						newIndexPage.insertKey(upEntry.key, ((IndexData) upEntry.data).getData());
					}
					// Goes to the currentIndexPage
					else {
						currentIndexPage.insertKey(upEntry.key, ((IndexData) upEntry.data).getData());
					}
		...
		// Page Type == Leaf
			...
			// Current Leaf Node has available space for the new entry
			...
			// No available space on the current leaf node,
			// create a new leaf page and distribute entries
			...
			// Transferring all the records to newLeafPage
			for (tmpkeyDataEntry = currentLeafPage
					.getFirst(delRID); tmpkeyDataEntry != null; tmpkeyDataEntry = currentLeafPage.getFirst(delRID)) {
			...
			// Distributing records between newLeafPage and currentLeafPage
			for (tmpkeyDataEntry = newLeafPage.getFirst(delRID); newLeafPage.available_space() < currentLeafPage
					.available_space(); tmpkeyDataEntry = newLeafPage.getFirst(delRID)) {
				lastEntry = tmpkeyDataEntry;
			...
			if (BT.keyCompare(key, lastEntry.key) < 0
					&& currentLeafPage.available_space() < newLeafPage.available_space()) {
				newLeafPage.insertRecord(lastEntry.key, ((LeafData) lastEntry.data).getData());
				PageId tempPageId = currentLeafPage.getCurPage();
				int slotNum = (int) currentLeafPage.getSlotCnt() - 1;
				RID tempRid = new RID(tempPageId, slotNum);
				currentLeafPage.deleteSortedRecord(tempRid);

			}

			// Positive so goes to the newLeafPage
			if (BT.keyCompare(key, lastEntry.key) >= 0) {
				newLeafPage.insertRecord(key, rid);
			}
			// Negative so goes to the currentLeafPage
			else {
				currentLeafPage.insertRecord(key, rid);
			}
	...
	private boolean NaiveDelete(KeyClass key, RID rid)
		...
		// finding the first page and RID of keys
		BTLeafPage leafPage = findRunStart(key, curRid);
		...
		while (true) {
			while (entry == null) {
				// Go to the next right page
				PageId nextPage = leafPage.getNextPage();

				// unpin leafPage as we move to the next page
				unpinPage(leafPage.getCurPage());

				int nextPageId = nextPage.pid;

				// if the next page is invalid we return false.
				if (nextPageId == INVALID_PAGE) {
					return false;
				}

				// since next page is not invalid initialize leafpage to next page
				// and pin it to next page.
				leafPage = new BTLeafPage(pinPage(nextPage), headerPage.get_keyType());
				entry = leafPage.getFirst(new RID());
			}
			// checking the key and entry.key if positive value break
			if (BT.keyCompare(key, entry.key) > 0)
				break;

			// check if the key is found and if true delete.
			...
			// getting the page and key for the new leafpage.
			Page page = pinPage(nextPage);
			int keyNumber = headerPage.get_keyType();
			leafPage = new BTLeafPage(page, keyNumber);
